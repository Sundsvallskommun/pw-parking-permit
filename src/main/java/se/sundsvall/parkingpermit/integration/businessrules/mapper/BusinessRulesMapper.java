...
public final class BusinessRulesMapper {
	...
	public static RuleEngineRequest toRuleEngineRequest(Errand errand) {
	...
	private static List<Fact> toFacts(List<String> keys, Errand errand) {
		return keys.stream()
	...
	private static String getExtraParameterValue(Errand errand, String key) {
		return ofNullable(errand.getExtraParameters()).orElse(emptyList()).stream()
			...
			.findFirst()
			.flatMap(extraParameter -> extraParameter.getValues().stream().findFirst())
			.orElse(null);
	}

	private static boolean isDriver(Errand errand) {
		return APPLICANT_DRIVER.equals(getExtraParameterValue(errand, CASEDATA_KEY_APPLICATION_APPLICANT_CAPACITY));
	}
