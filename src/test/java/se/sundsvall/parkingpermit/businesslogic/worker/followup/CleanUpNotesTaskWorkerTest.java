...
class CleanUpNotesTaskWorkerTest {
	...
	void execute() {
		...
		when(caseDataClientMock.getNotesByErrandId(ERRAND_ID, INTERNAL.getValue())).thenReturn(notes);
		// Act
		worker.execute(externalTaskMock, externalTaskServiceMock);

		// Assert and verify
		verify(caseDataClientMock).getNotesByErrandId(ERRAND_ID, INTERNAL.getValue());
	...
	void executeThrowsException() {
		...
		// Mock
		doThrow(thrownException).when(caseDataClientMock).deleteNoteById(1L);
