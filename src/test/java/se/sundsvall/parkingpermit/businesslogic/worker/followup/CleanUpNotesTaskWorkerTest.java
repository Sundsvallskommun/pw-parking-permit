...
class CleanUpNotesTaskWorkerTest {
	...
	void execute() {
		//Arrange
		final var notes = List.of(new NoteDTO().id(1L).noteType(INTERNAL), new NoteDTO().id(2L).noteType(INTERNAL));
		...
		when(caseDataClientMock.getNotesByErrandId(MUNICIPALITY_ID, ERRAND_ID, INTERNAL.getValue())).thenReturn(notes);
		// Act
		worker.execute(externalTaskMock, externalTaskServiceMock);

		// Assert and verify
		verify(caseDataClientMock).getNotesByErrandId(MUNICIPALITY_ID, ERRAND_ID, INTERNAL.getValue());
		verify(caseDataClientMock).deleteNoteById(MUNICIPALITY_ID, 1L);
		verify(caseDataClientMock).deleteNoteById(MUNICIPALITY_ID, 2L);
	...
	void executeThrowsException() {
		...
		// Mock
		doThrow(thrownException).when(caseDataClientMock).deleteNoteById(MUNICIPALITY_ID, 1L);
